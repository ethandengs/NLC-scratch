...
do $$
begin
    -- Copy owner_id to user_id logic (Same as V3)
...
do $$
begin
    -- Users Policies (Casting auth.uid() to text because users.id is text)
    ...
    -- Sheep Policies (user_id is UUID, so no cast needed)
